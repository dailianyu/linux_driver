...
/* 	
	this is a preface program  used to intruduce concurrency and race condition.
...
my_pipe_init(void )
{
	...
	if (devno_major > 0) {
		...
		ret = register_chrdev_region(devno , 1, "mypipe30");
	} else {
		ret = alloc_chrdev_region(&devno, 0, 1, "mypipe30");
	...
	//step 3:  创建设备节点
	my_pipe_class  = class_create(THIS_MODULE, "my_pipe_class_30");
	...
	my_pipe_device = device_create(my_pipe_class,NULL, devno, NULL,"mypipe30");
