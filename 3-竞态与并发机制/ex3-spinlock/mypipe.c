...
/* 	
	this program have bugs, please point out.
...
ssize_t my_pipe_write (struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	spin_lock(&pbuf->lock);  /*这个地方用自旋锁有问题，longer*/
...
my_pipe_init(void )
{
	...
	if (devno_major > 0) {
		...
		ret = register_chrdev_region(devno , 1, "mypipe33");
	} else {
		ret = alloc_chrdev_region(&devno, 0, 1, "mypipe33");
	...
	//step 3:  创建设备节点
	my_pipe_class  = class_create(THIS_MODULE, "my_pipe_class33");
	...
	my_pipe_device = device_create(my_pipe_class,NULL, devno, NULL,"mypipe33");
