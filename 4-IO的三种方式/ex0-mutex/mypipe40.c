...
my_pipe_init(void )
{
	...
	if (devno_major > 0) {
		...
		ret = register_chrdev_region(devno , 1, "mypipe40");
	} else {
		ret = alloc_chrdev_region(&devno, 0, 1, "mypipe40");
	...
	//step 3:  创建设备节点
	my_pipe_class  = class_create(THIS_MODULE, "my_pipe_class40");
	...
	my_pipe_device = device_create(my_pipe_class,NULL, devno, NULL,"mypipe40");
