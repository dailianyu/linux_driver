...
void SectorErase(U32 sector)
{
	...
	*(volatile U16 *)sector = 0x3030;
	...
	while(1)
	{
		...
		/* During internal Erase operation, any attempt to read
		DQ7 will produce a '0'. Once the internal Erase opertion
		is completed  ,DQ7 will produce a '1'. zhoulong*/
	...
	{	
		...
		*(volatile U16 *)ProgStart = j;	
		...
		while(1)
		{
			...
			/*D7 == D7 . When the SST39LF/VF160 are in the internal Program
			operation, any attempt to read DQ7 will produce the complement of 
			the ture data. Once the Program opertion is completed, DQ7 will 
			produce true data. zhoulong */
...
char FlashWriteU16( U32 Addr, U16 Value)
{
	U16 i;	
	U32 timeout;
	...
	*(volatile U16 *)Addr = Value;	
	...
	while(1)
	{
		...
		if(i!=*(volatile U16 *)Addr&0x40)		//D6 == D6
			continue;
		if((*(volatile U16 *)Addr&0x80)==(Value&0x80))	return 1;
	}
